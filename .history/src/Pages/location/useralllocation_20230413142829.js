...
const UserAllLocation = (props) => {
  ...
  const FetchUserLocation = async (token) => {
    await axios
      ...
      .get(`${BACKEND_URL}/locations?userID=${userid}&date_range=${(new Date() - 1).toISOString().slice(0, 10)},${(new Date() + 1).toISOString().slice(0, 10)}`, {
        headers: {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      console.log((new Date() - 1).toISOString().slice(0, 10));
      console.log((new Date() + 1).toISOString().slice(0, 10));
    } else {
      navigate("/", {
        state: { error: "Something Went Wrong! Please Try Again", continueURL: location.pathname },
