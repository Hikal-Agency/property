      ...
      {meetingData.meeting.map(meeting => (
        <>
          <MarkerF
            ...
            position={{ lat: meeting.meetingDetails.lat, lng: meeting.meetingDetails.lng}}
            icon={{
              url: "/meetingpin.svg",
          ...
          {selected ? (
            <InfoWindow             
              position={{ lat: selected.meetingDetails.lat, lng: selected.meetingDetails.lng}}
              onCloseClick={
                () => {setSelected(null);
              }}
            >
          ...
          >
            <div>
              <h1>{meeting.leadDetails.leadName}</h1>
              <h1 className="font-semibold">{meeting.leadDetails.project}&nbsp;{meeting.leadDetails.enquiryType}&nbsp;{meeting.leadDetails.leadType}</h1>
              <h1>{meeting.meetingDetails.meetingTime}&nbsp;{meeting.meetingDetails.meetingDate}</h1>
              <h1 className="font-semibold">{meeting.meetingDetails.meetingBy}</h1>
            </div>
          </InfoWindow>
        </>
      ))}
