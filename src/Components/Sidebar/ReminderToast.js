    ...
    <div className="flex flex-col items-center rounded py-7 px-4">
      ...
      <div className="p-4 mb-4 bg-[#ecbf3f] flex items-center justify-center">
        {type === "reminder" ? (
          ...
          <MdOutlineAssignmentTurnedIn style={{ color: "white" }} size={28} />
        ) : (
          <></>
        )}
      </div>
      {type === "reminder" ? (
        <p className="text-center mb-4">
          A friendly reminder that you scheduled follow up with{" "}
          <strong>{leadName}</strong> is just 5 minutes away. Get ready to make
          the most of your conversation!
        </p>
      ) : type === "lead_assigned" ? (
        <p>Lead `{leadName}` is assigned to you!</p>
      ) : (
        <></>
      )}
